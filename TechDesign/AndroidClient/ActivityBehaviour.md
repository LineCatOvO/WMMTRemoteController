...
**布局从外部存储文件夹读取**：所有布局从应用外部存储文件夹读取，一个JSON为一个布局，JSON的metadata部分描述布局元数据（如名称、描述等）。

**布局管理页面**：从应用外部存储文件夹读取所有JSON，验证合法性，然后显示给用户。

**新用户布局**：新用户没有布局时，可以在创建页面选择一个预定义布局，预定义布局存放在 `predefined-profiles` 目录。

**外部存储路径**：`Context.getExternalFilesDir(null)/"layout"/{layoutId}/main.json`
